...
export const Passengers = ({ state, send }) => {
  ...
  const submit = (e) => {
    e.preventDefault();
    send('ADD', { newPassenger: value })    // 2do parametro enviamos datos del pasajero
    changeValue('');                        // enviado el dato, seteamos en ''              
  }

  const { passengers } = state.context;     // Obtengo los pasajeros guardados del contexto
